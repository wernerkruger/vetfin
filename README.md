...
```bash
cd api
cp .env.example .env    # add Plaid sandbox keys
npm install
...
| `DATA_ENCRYPTION_KEY` | API | Encrypts Plaid tokens at rest (≥16 chars; **required** in production) |
...
Production safety checks (`NODE_ENV=production`):
